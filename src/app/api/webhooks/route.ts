...
//       // Build user object from Clerk event data
//       const user: Partial<User> = {
//         id: evt.data.id,
//         name: `${evt.data.first_name} ${evt.data.last_name}`,
//         email: evt.data.email_addresses[0].email_address,
...
//       // If the user exists, preserve their role (e.g., manually set to "ADMIN" in Prisma)
//       const preservedRole = existingUser?.role || "USER";

//       // Upsert user, preserving role if exists
//       const dbUser = await db.user.upsert({
//         where: { email: user.email },
//         update: {
//           ...user,
